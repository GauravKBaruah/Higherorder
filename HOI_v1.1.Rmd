...
gausquad<-function(index,m,params,interaction,points){
  ...
  if(interaction=="HOI"){
    ...
    for (i in 1: points){
      ...
      dat2[i]<- sum((z1[i]-m[index$i])*(0.5*(tanh((z1[i]-z2)/params$w)+1))*w1[i]*w2)
    }
    return(list(Hjk= sum(temp2), bjk = sum(dat2)) )
...
eqs<- function(time, x, parms){
  ...
  if(parms$interaction == "HOI"){
    ...
    dmdt<-parms$h2*sum(parms$g*(parms$theta^2-
      ...
      parms$h2*2*parms$g*m^2*(m-parms$theta)
  ...
  # if the interaction model is pairwise in nature
    ...
    for(r in 1:S){
      for(l in 1:S){
        ...
        temp<-gausquad(index=indices,m = m,params=parms,interaction=parms$interaction, points=5)
    ...
    dndt<- n*(sum(parms$g*(parms$theta^2-2*m*parms$theta+parms$sigma^2+ m^2)*n)*(2*t_m%*%n/(sum(n)*sum(n)))) -n*parms$g*(parms$theta^2-2*m*parms$theta+parms$sigma^2+m^2)
    
    #dndt<-0
    dmdt<-parms$h2*sum(parms$g*(parms$theta^2-
                                  2*m*parms$theta+parms$sigma^2+
                                  m^2)*n)*2*B%*%n/(sum(n)*sum(n))-parms$h2*2*parms$g*m^2*(m-parms$theta)
...
```{r pairwise, fig.align="center", fig.height=6.0, fig.width=4.5, message=FALSE, warning=FALSE, out.width="90%"}
...
m<-runif(S,-0.1,0.1) # initial random trait values
sigma<-runif(S,0.002,0.005) #trait vairance
w=1 # parameter that captures the strength of competition
h2<-runif(S,0.15,0.15) #heritabilies of mean trait value
g<-0.001 #strength in the mortality curve
...
times <- seq(0, 1e9, length=1000)

ode(func=eqs, y=ic, parms=pars, times=times) %>% 
  organize_results(pars) %>% plot_all(1e9)
```
...
m<-runif(S,-0.1,0.1) # initial random trait values
sigma<-runif(S,0.02,0.05) #trait vairance
w=1 # parameter that captures the strength of competition
h2<-runif(S,0.15,0.15) #heritabilies of mean trait value
g<-0.001 #strength in the mortality curve
theta<-0 #optimum mortality value
...
```{r ind variation, fig.align="center", fig.height=3, fig.width=4, message=FALSE, warning=FALSE, out.width="50%"}
...
m<-runif(S,-1,1) # initial random trait values
sigma<-runif(S,0.0001,0.0005) #trait vairance
w=1 # parameter that captures the strength of competition
h2<-runif(S,0.15,0.15) #heritabilies of mean trait value
g<-0.0005 #strength in the mortality curve
theta<-0 #optimum mortality value
interaction = "pairwise" #pairwise or HOI
...
## set up factorial numerical experiment, with seven different intraspecific
## std dev levels, and five replicates for each
fact <- expand.grid("sigma"= c("low","medium","high"),
                    "random_seed"=4127+(1:20)*100) %>%
  as_tibble %>% ## convert data frame to tibble
  mutate(`sp_richness`=0,
         `CV_Trait`= 0)


for (r in 1:nrow(fact)) { ## for each parameterization:
  ## set up simulation with intraspecific variation
  if(fact$sigma[r] == "low"){ sigma = runif(S, 0.0004,0.003)}
  else if(fact$sigma[r] == "medium") {sigma= runif(S,0.003,0.009) }
  else if(fact$sigma[r] == "high") {sigma = runif(S, 0.01,0.05)}
  
  pars<-list( S=S,h2=h2,w=w,sigma=sigma,theta=theta, 
            g=g,interaction=interaction, threshold = 1e-9) #
                 #rep(fact1$sigma[r], S)+0.0003*runif(S, -1, 1))
 
  muinit <- runif(S, -0.5, 0.5) ## initial species trait means
  sol <- ode(func=eqs, y=c(x0,m), parms=pars, times=times) %>%
    organize_results(pars) %>% ## solve ODEs & put results in tidy table
    filter(time==tmax) %>% ## keep data for only the final point in time
    filter(n>1e-9) ## keep only those species with density above a threshold
  fact$sp_richness[r]=nrow(sol)
  fact$CV_Trait[r] = IntravarCV(sol)
  print(r)

}

plot.rich<-ggplot(data= fact, aes(y = sp_richness,
                                  x = sigma, color= sigma )) +
  geom_boxplot(alpha=0.5)+
  geom_point(pch = 16,alpha=0.5, position = position_jitterdodge())+ 
  ylab("Species richness")+
  theme_classic()+ylim(c(0,16))+xlab("")+
...
plot.cv<-ggplot(data= fact, aes(y = CV_Trait,
                                  x = sigma, color= sigma )) +
  geom_boxplot(alpha=0.5)+
  geom_point(pch = 16,alpha=0.5, position = position_jitterdodge())+ 
  ylab("Species richness")+
  theme_classic()+xlab("")+ylim(c(-200,2000))+
  theme(plot.title = element_text(size = 10,   face = "bold"),
        text = element_text(size = 10 ),
        axis.title = element_text(face="bold"),legend.position = "none") 
plot.cv
